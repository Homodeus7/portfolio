...
<template>
  <div class="flex flex-col items-center gap-4 transition-all duration-500">
    ...
    <div class="flex gap-8">
      <!-- Номера этажей -->
      <div class="grid gap-4 rtl" :style="`grid-template-rows: repeat(${props.floors}, 40px);`">
        <div
      ...
      >
        <template v-for="(floor, floorIndex) in apartmentNumbers" :key="`floor-${floorIndex}`">
          <template v-for="(apartment, unitIndex) in floor" :key="`apt-${floorIndex}-${unitIndex}`">
            <div
              class="bg-[#41b883] w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm cursor-pointer transition-colors duration-300"
            >
              {{ apartment }}
            </div>
          </template>
